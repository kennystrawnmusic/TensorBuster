...
class SessionContextManager(Middleware):
    ...
    def add_user_command(self, session_id: str, prompt: str) -> None:
        ...
        self.session_history[session_id].append({"role": "user", "content": prompt})
    ...
    def build_prompt_context(self, session_id: str, tokenizer=None) -> str:
        ...
        messages = [{"role": "system", "content": self.base_instructions}]

        # Add conversation history
        messages.extend(self.get_session_history(session_id))

        # Apply tokenizer template
        context = tok.apply_chat_template(  # type: ignore
    ...
    async def on_request(self, context, call_next):
        ...
        if hasattr(context, "request_body"):
            ...
            request_body = context.request_body  # type: ignore[attr-defined]
            if isinstance(request_body, dict):
                user_command = request_body.get("content") or request_body.get(
                    "message"
                )
        ...
        if (
            session_id
            and user_command
            and isinstance(user_command, str)
            and user_command.strip()
        ):
    ...
    async def on_response(self, context, call_next):
        ...
        if isinstance(result, dict):
            agent_response = (
                result.get("content") or result.get("response") or result.get("message")
            )
        ...
        if (
            agent_response
            and isinstance(agent_response, str)
...
class SessionTracker(Middleware):
    async def on_initialize(self, context: MiddlewareContext, call_next):
        ...
        # context.fastmcp_context is available in the middleware context
        if context.fastmcp_context and context.fastmcp_context.session_id:
            session_id = context.fastmcp_context.session_id
            if session_id not in consts.SESSIONS:
                consts.SESSIONS.append(session_id)
