...
def _fetch_latest_wheel(package_name):
    ...
    if "torchao" in package_name:
        ...
        if current_os == "darwin":
            ...
            base_url = (
                f"https://download-r2.pytorch.org/whl/nightly/cu130/{package_name}/"
            )
    ...
    try:
        ...
        if not valid_wheels:
            raise ValueError(
                f"No compatible {current_os} wheels found for {package_name}"
            )
        ...
        raise RuntimeError(
            f"Failed to resolve compatible nightly wheel for {package_name}: {e}"
        )
...
def get_requires_for_build_wheel(config_settings=None):
    return _get_nightly_requirements() + _orig.get_requires_for_build_wheel(
        config_settings
    )
...
def prepare_metadata_for_build_wheel(metadata_directory, config_settings=None):
    ...
    try:
        dist_info_dir = _orig.prepare_metadata_for_build_wheel(
    ...
    new_lines = [
        l for l in lines if not any(f"Requires-Dist: {n}" in l for n in nightly_names)
    ]
